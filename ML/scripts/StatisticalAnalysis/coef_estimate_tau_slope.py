...
def statistics_plot_dev(coefficient):
    ...
    for indx, coef in enumerate(coefficients[:,1]):
        ...
        plt.vlines(indx,coef,mean_value[1],colors='r',linestyles= 'dashed')
        plt.text(indx+0.25, coef-diff, r'%.2f' % diff,
        {'color': 'red', 'fontsize': 10, 'ha': 'center', 'va': 'center'})

    plt.savefig(data_path+"residual_of_mean_dev.png")
    plt.clf()
