...
args.add_argument("--regions", default=None, type=str, help="Path to tab-delimited source file (see find_sources.py) that holds source RA and Dec in 2nd and 3rd column respectively, within which to only include GLEAM point sources.")
...
args.add_argument("--exclude", default=False, action='store_true', help="If providing regions via --regions, exclude souces within masks, rather than only including sources within masks per default behavior.")
...
if __name__ == "__main__":
    ...
    basename = os.path.join(a.outdir, "{}{}_model.cl".format(os.path.basename(a.gleamfile[:-5]),a.ext))
