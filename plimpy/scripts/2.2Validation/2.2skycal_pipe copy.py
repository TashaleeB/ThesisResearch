...
def pbcorr():
    ...
    _fitsfiles = ["{}.fits".format(modelstem)]
...
cf = utils.load_config(a.param_file)#yaml.load(a.param_file, Loader=yaml.FullLoader)
...
def gen_model(**kwargs):
    ...
    if p.pbcorr:
        ...
        cmd = ["/Users/tashaleebillings/casa_imaging/scripts/pbcorr.py", "--lon", p.longitude, "--lat", p.latitude, "--time", p.time, "--pols"] \
               ...
               + ["--outdir", p.out_dir, "--multiply", "--beamfile", p.beamfile, "fitsfiles", ["{}.fits".format(modelstem)]]
        if p.overwrite:
...
if params['gen_model']:
    ...
    model = gen_model(**dict(list(algs['gen_model'].items()) + list(global_vars(varlist).items())+list(cf['obs'].items())))
...
def calibrate(**cal_kwargs):
    kwargs = dict(list(algs['gen_model'].items()) + list(global_vars(varlist).items())+list(cf['obs'].items()))
    p = Dict2Obj(**kwargs)
...
if params['di_img']:
    ...
    time = datetime.utcnow()
    utils.log("\n{}\n...Starting DI_IMG: {}\n".format("-"*60, time), f=lf, verbose=verbose)
    img_kwargs = copy.deepcopy(dict((list(algs['imaging'].items())) + list(algs['di_img'].items())))
